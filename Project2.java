...
public class Project2 {
	...
	public static void main(String[] args) throws IOException {
		...
		String inputFolder = Project2.class.getClassLoader().getResource("examples/workload/planetlab/20110403").getPath();
	...
	try {
		...
		//PowerVmSelectionPolicy vmSelectionPolicy = new PowerVmSelectionPolicyMaximumCorrelation(new PowerVmSelectionPolicyMinimumMigrationTime());

PowerVmSelectionPolicy vmSelectionPolicy = new PowerVmSelectionPolicyEco(new PowerVmSelectionPolicyMinimumMigrationTime());
			List<PowerHost> hostList1 = createHostList(800);

	

PowerDatacenter datacenter1 = (PowerDatacenter) createDatacenter("Datacenter1",PowerDatacenter.class,hostList1,new PowerVmAllocationPolicyEco(hostList1,vmSelectionPolicy,0.8));

List<PowerHost> hostList2 = createHostList(500);


PowerDatacenter datacenter2 = (PowerDatacenter) createDatacenter("Datacenter2",PowerDatacenter.class,hostList2,new PowerVmAllocationPolicyEco(hostList2,vmSelectionPolicy,0.9));

List<PowerHost> hostList3 = createHostList(600);


PowerDatacenter datacenter3 = (PowerDatacenter) createDatacenter("Datacenter3",PowerDatacenter.class,hostList3,new PowerVmAllocationPolicyEco(hostList3,vmSelectionPolicy,0.9));
...
PowerDatacenterBroker broker = createBroker();
			...
			List<Cloudlet> cloudletList = createCloudletListPlanetLab(brokerId, inputFolder);
...
CloudSim.terminateSimulation(10000.0);
	...
	public static Datacenter createDatacenter(
		...
		try {
			datacenter = datacenterClass.getConstructor(String.class,DatacenterCharacteristics.class,VmAllocationPolicy.class,List.class,Double.TYPE).newInstance(name,characteristics,vmAllocationPolicy,new LinkedList<Storage>(),300.0);
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(0);
		}
...
public static List<Cloudlet> createCloudletListPlanetLab(int brokerId, String inputFolderName)
		...
		for (int i = 0; i < files.length; i++) {
			...
			try {
				cloudlet = new Cloudlet(i,2500 * (int) 24 * 60 * 60,1,fileSize,outputSize,new UtilizationModelPlanetLabInMemory(files[i].getAbsolutePath(),300.0), utilizationModelNull, utilizationModelNull);
			} catch (Exception e) {
				e.printStackTrace();
				System.exit(0);
			}
	...
	public static List<Vm> createVmList(int brokerId, int vmsNumber) {
		...
		for (int i = 0; i < vmsNumber; i++) {
			int vmType = i / (int) Math.ceil((double) vmsNumber / 4);
			vms.add(new PowerVm(i,brokerId,VM_MIPS[vmType],VM_PES[vmType],VM_RAM[vmType],VM_BW,VM_SIZE,1,"Xen",new CloudletSchedulerDynamicWorkload(VM_MIPS[vmType], VM_PES[vmType]),300.0));
		}
		return vms;
	}
	...
	public static List<PowerHost> createHostList(int hostsNumber) {
		...
		for (int i = 0; i < hostsNumber; i++) {
			...
			hostList.add(new PowerHostUtilizationHistory(i,new RamProvisionerSimple(HOST_RAM[hostType]),new BwProvisionerSimple(HOST_BW),HOST_STORAGE,peList,new VmSchedulerTimeSharedOverSubscription(peList),HOST_POWER[hostType]));
		}
		return hostList;
	}
